$(document).ready(function () {
  ...
  function vgaDecoder() {
    this.resX = 640;
    this.resY = 480;
    this.pixelClockRate = 25; // in MHz

    this.backPorchX = 48; // From: http://tinyvga.com/vga-timing/640x480@60Hz
    this.backPorchY = 33;
    ...
    this.log = function (text) {
      if (this.shouldLog) console.log(text);
    };
    ...
    this.handleLines = function (lineIndex) {
      ...
      for (var i = 0; i < numLinesPerIteration; i++) {
        ...
        // Get outta here
        if (lineIndex >= this.file_lines.length - 1) break;
        ...
        matches = this.file_lines[lineIndex].match(
          /^([0-9]+) (fs|ps|ns|us|ms|sec|min|hr): (0|1|U|X|Z) (0|1|U|X|Z) ((?:0|1|U|X|Z)+) ((?:0|1|U|X|Z)+) ((?:0|1|U|X|Z)+)/i,
        );
        ...
        if ((matches || []).length > 0) {
          ...
          // Assemble data
          sim_time_secs = this.timeConversion(
            matches[2],
            "sec",
            parseInt(matches[1]),
          );
          ...
          // Now add the time
          this.timeSinceLastPixel += parseFloat(
            (sim_time_secs - this.lastPacketTime).toFixed(20),
          );
          ...
          if (this.last_hSync === 0 && hsync == 1) {
            ...
            if (this.backPorchYCount >= this.backPorchY) this.vCounter++;
          ...
          if (this.imageArray.length > 0) {
            ...
            if (
              this.timeSinceLastPixel >=
                parseFloat(
                  (1 / (this.pixelClockRate * 1000000) - tolerance).toFixed(20),
                ) &&
              this.timeSinceLastPixel <=
                parseFloat(
                  (1 / (this.pixelClockRate * 1000000) + tolerance).toFixed(20),
              ...
              if (
                this.backPorchXCount >= this.backPorchX &&
                this.backPorchYCount >= this.backPorchY
              ) {
              ...
              if (this.backPorchXCount >= this.backPorchX) this.hCounter++;
      ...
      if (this.imageArray.length > 0) {
        ...
        this.canvas_manager.putImageData(
          this.imageArray[this.imageArray.length - 1],
          0,
          0,
          false,
      ...
      if (!this.stopHandlingLines && lineIndex < this.file_lines.length - 1) {
        ...
        this.log(
          "Iterating through all lines took (ms): " + this.handeLinesExecTime,
        );

        // Tell them if we got any parsing errors
        $(this.canvas_manager.element).trigger("progressDone", {
          numLinesSkipped: this.numLinesSkipped,
        });
      }

      // Add progress
      $(this.canvas_manager.element).trigger("progressUpdate", {
        lineIndex: lineIndex,
      });
    };
        ...
        for (var f = 0; f < this.imageArray.length; f++) {
          ...
          if ($("#vga-frame-" + f).length === 0) {
            ...
            this.frameHolder.append(
              '<div class="vga-frame">' +
                f +
                ': <br /><canvas id="vga-frame-' +
                f +
                '" class="vga-frame-canvas no-smoothing" data-frame-number="' +
                f +
                '"></canvas></div>',
            );
          }
    ...
    this.bin_to_color = function (bin) {
      ...
      // (value/bitdepth)*255
      return (
        (parseInt(bin, 2) / parseInt(Array(bin.length).join("1"), 2)) * 255
      );
    };
    ...
    this.timeConversion = function (unitFrom, unitTo, value) {
      ...
      return parseFloat(
        (
          (time_unit_dictionary[unitFrom] / time_unit_dictionary[unitTo]) *
          value
        ).toFixed(20),
      );
    };
  ...
  $("#vga-canvas").on("progressUpdate", function (e, data) {
    $("#log-form-progress").html(
      data.lineIndex + 1 + " / " + my_vgaDecoder.file_lines.length,
    );
  });
  ...
  $("#vga-canvas").on("progressDone", function (e, data) {
    if (data.numLinesSkipped > 0) {
      $("#log-form-status").html(
        "Skipped " +
          data.numLinesSkipped +
          " lines. Error parsing those lines.",
      );
    }
  });
  ...
  $("#log-form").on("click", "input[type=submit]", function (e) {
    if (this.value == "Stop") {
      ...
      if (my_vgaDecoder.shouldLog)
        console.log(
          "bp x: " +
            my_vgaDecoder.backPorchX +
            " - bp y: " +
            my_vgaDecoder.backPorchY,
        );
  ...
  $("#vga-frame-holder").on("click", ".vga-frame", function () {
    my_canvasMan.putImageData(
      my_vgaDecoder.imageArray[
        parseInt($(this).find(".vga-frame-canvas").attr("data-frame-number"))
      ],
      0,
      0,
      false,
